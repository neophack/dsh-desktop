...
Packaged macOS and Windows applications query the GitHub Releases API of [`neophack/dsh-desktop`](https://github.com/neophack/dsh-desktop) (`https://api.github.com/repos/neophack/dsh-desktop/releases/latest`) 60 seconds after startup and every six hours after a completed check. Each no-cache request has a 15-second deadline and shares one in-flight operation with the **Check for Updates…** tray command; it carries no Desktop installation identifiers. Stable accepts only the canonical release SemVer of the latest published release tag and never discovers a Beta tag. Background failures and non-newer versions remain silent; a manual check always opens a native result dialog. Development, unpackaged, and Linux launches do not download an installer.
...
Release operators must attach both platform installer assets to the `v{version}` GitHub release before making a stable version discoverable. The updater pins a download to the checked release tag, so a missing tag, a missing installer asset, or a mismatched version produces no Desktop prompt.
