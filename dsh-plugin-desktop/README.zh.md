...
打包后的 macOS 与 Windows 应用会在启动 60 秒后查询 [`neophack/dsh-desktop`](https://github.com/neophack/dsh-desktop) 的 GitHub Releases API（`https://api.github.com/repos/neophack/dsh-desktop/releases/latest`），并在每次检查完成六小时后再次查询。每次 no-cache 请求的期限为 15 秒，与托盘中的 **Check for Updates…** 命令共用一个 in-flight operation，且不携带任何 Desktop 安装标识。稳定版只接受最新已发布 release tag 的规范正式 SemVer，绝不会发现 Beta tag。后台失败和非更新版本保持静默；手工检查一定会显示原生结果对话框。开发运行、未打包启动与 Linux 不会下载安装包。
...
Release operator 必须先把两个平台的安装包资产附到 `v{版本}` GitHub release 上，再让稳定版本可被发现。更新器会把下载固定到已确认的 release tag，因此 tag 缺失、安装包资产缺失或版本不匹配时，Desktop 不会显示任何提示。
